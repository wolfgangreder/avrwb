...
//    }
  ...
  private native boolean testRead(String dev,
                                  int type);
  ...
  private final String[] getValidPortPrefixes(String CandidatePortPrefixes[])
  {
    ...
    System.arraycopy(ValidPortPrefixes,
                     0,
                     returnArray,
                     0,
                     i);
   ...
   */
  private void checkSolaris(String PortName,
                            int PortType)
  {
    char p[] = {91};
    for (p[0] = 97; p[0] < 123; p[0]++) {
      if (testRead(PortName.concat(new String(p)),
                   PortType)) {
    ...
    if (CandidateDeviceNames != null && ValidPortPrefixes != null) {
      for (i = 0; i < CandidateDeviceNames.length; i++) {
        for (p = 0; p < ValidPortPrefixes.length; p++) {
          ...
          String PortName;
          if (osName.toLowerCase().indexOf("windows") == -1) {
            PortName = new String(deviceDirectory + C);
          } else {
            PortName = new String(C);
          }
          if (debug) {
          ...
          if (osName.equals("Solaris") || osName.equals("SunOS")) {
            checkSolaris(PortName,
                         PortType);
          } else if (testRead(PortName,
                              PortType)) {
            CommPortIdentifier.addPortName(
                    PortName,
                    PortType,
  ...
  private void addSpecifiedPorts(String names,
                                 int PortType)
  {
 ...
 /*
    ...
    registerValidPorts(CandidateDeviceNames,
