...
final public class RXTXPort extends SerialPort
{
   ...
   * @throws gnu.io.PortInUseException err
   * @see gnu.io.SerialPort
   */
   ...
   * @param s stopbits
   * @param p parity
   * @throws UnsupportedCommOperationException err
  ...
  /**
   * <p>
   * readTerminatedArray.</p>
   *
   * @param b an array of byte.
   * @param off a int.
   * @param len a int.
   * @param t an array of byte.
   * @return a int.
   * @throws java.io.IOException if any.
   */
                           ...
                           boolean state)
  {
    ...
    switch (event) {
      ...
      case SerialPortEvent.OUTPUT_BUFFER_EMPTY:
        if (debug_events) {
          z.reportln(
                  "OUTPUT_BUFFER_EMPTY " + monThread.Output + ")");
        }
        break;
  ...
  class SerialOutputStream extends OutputStream
  {
            ...
            throws IOException
    {
      ...
      if (debug_write) {
        z.
                reportln("Entering RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " + ") " /* + new String(send) */);
      }
      if (fd == 0) {
        throw new IOException();
      }
      ...
      try {
        ...
        if (debug_write) {
          z.
                  reportln("Leaving RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " + ") " /* + new String(send) */);
        }
      } catch (IOException e) {
        IOLocked--;
        throw e;
            ...
            throws IOException
    {
      ...
      try {
        ...
        if (debug_read_results) {
          z.
                  reportln(
                          "RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") returned " + result
                          + " bytes" /* + new String(b) */);
        }
        return (result);
      } finally {
   ...
   * @throws gnu.io.UnsupportedCommOperationException err This will not behave as expected with custom speeds
   */
   ...
   * @throws gnu.io.UnsupportedCommOperationException err
   */
          ...
          throws UnsupportedCommOperationException
  {
    if (debug) {
      z.reportln(
              "RXTXPort:staticSetSerialPortParams( " + f + " " + b + " " + d + " " + s + " " + p);
    }
