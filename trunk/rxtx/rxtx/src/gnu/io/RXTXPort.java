 ...
 */
final public class RXTXPort extends SerialPort implements AutoCloseable
{
  ...
  /**
   * a pointer to the event info structure used to share information between threads so write threads can send output buffer empty
  ...
  @Override
  public synchronized void setSerialPortParams(int b,
                                               int d,
                                               int s,
                                               int p)
          throws UnsupportedCommOperationException
  {
    if (nativeSetSerialPortParams(b,
                                  d,
                                  s,
                                  p)) {
  ...
  {
    timeout = -1;
    NativeEnableReceiveTimeoutThreshold(timeout,
                                        threshold,
                                        InputBuffer);
  }
  ...
  public void enableReceiveTimeout(int time)
  {
    if (time >= 0) {
      timeout = time;
      NativeEnableReceiveTimeoutThreshold(time,
                                          threshold,
                                          InputBuffer);
    } else {
  ...
  protected native void writeByte(int b,
                                  boolean i) throws IOException;

  protected native void writeArray(byte b[],
                                   int off,
                                   int len,
                                   boolean i)
          throws IOException;
  ...
  protected native int readArray(byte b[],
                                 int off,
                                 int len)
          throws IOException;

  protected native int readTerminatedArray(byte b[],
                                           int off,
                                           int len,
                                           byte t[])
          throws IOException;
   ...
   *
   * This is important for applications that flicker the Monitor thread while keeping the port open. In worst case test cases this
   * loops once or twice every time.
   */
   ...
   * @param enable
   */
  private native void nativeSetEventFlag(int fd,
                                         int event,
                                         boolean flag);
    ...
    public void flush() throws IOException
    {
      ...
      try {
        ...
        if (nativeDrain(monThreadisInterrupted)) {
          sendEvent(SerialPortEvent.OUTPUT_BUFFER_EMPTY,
                    true);
        }
      } finally {
        synchronized (IOLockedMutex) {
  ...
  class SerialInputStream extends InputStream
  {
     ...
     * @see java.io.InputStream
     *
     * timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte
     * is available timeout > 0, threshold = 0, blocks until timeout occurs, returns -1 on timeout >0 >0 blocks until timeout,
     * returns - 1 on timeout, magnitude of threshold doesn't play a role. 0 >0 Blocks until 1 byte, magnitude of threshold
     * doesn't play a role
     */
    @Override
     ...
     *
     * timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte
     * is available >0 0 blocks until timeout occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes,
     * returns 0 on timeout 0 >0 blocks until reads threshold bytes
     */
    @Override
    @SuppressWarnings("NestedSynchronizedStatement")
...
Documentation is at http://java.sun.com/products/jdk/1.2/docs/api/java/io/InputStream.html#read(byte[], int, int)
     ...
     * timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte
     * is available >0 0 blocks until timeout occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes,
     * returns 0 on timeout 0 >0 blocks until either threshold # of bytes or len bytes, whichever was lower.
     */
    @Override
    @SuppressWarnings("NestedSynchronizedStatement")
     ...
     *
     * if the read encounters the two bytes, it will return and the array will contain the terminator. Otherwise read behavior
     * should be the same as read( b[], off, len ). Timeouts have not been well tested.
     */
    @SuppressWarnings("NestedSynchronizedStatement")
    public synchronized int read(byte b[],
                                 int off,
  ...
  class MonitorThread extends Thread
  {
    ...
    /**
     * Note: these have to be separate boolean flags because the SerialPortEvent constants are NOT bit-flags, they are just
  ...
  private native boolean nativeSetUartType(String type,
                                           boolean test)
          throws UnsupportedCommOperationException;
  ...
  public static void staticSetSerialPortParams(String f,
          ...
          throws UnsupportedCommOperationException
  {
    nativeStaticSetSerialPortParams(f,
                                    b,
                                    d,
                                    s,
                                    p);
   ...
   * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
   *
   * Open the port and set DSR. remove lockfile and do not close This is so some software can appear to set the DSR before
   * 'opening' the port a second time later on.
   *
   * @param port
   * @param flag
  ...
  public static boolean staticSetDSR(String port,
          ...
          throws UnsupportedCommOperationException
  {
    return (nativeStaticSetDSR(port,
                               flag));
  }
   ...
   * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
   *
   * Open the port and set DTR. remove lockfile and do not close This is so some software can appear to set the DTR before
   * 'opening' the port a second time later on.
   *
   * @param port
   * @param flag
  ...
  public static boolean staticSetDTR(String port,
          ...
          throws UnsupportedCommOperationException
  {
    return (nativeStaticSetDTR(port,
                               flag));
  }
   ...
   * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
   *
   * Open the port and set RTS. remove lockfile and do not close This is so some software can appear to set the RTS before
   * 'opening' the port a second time later on.
   *
   * @param port
   * @param flag
  ...
  public static boolean staticSetRTS(String port,
          ...
          throws UnsupportedCommOperationException
  {
    return (nativeStaticSetRTS(port,
                               flag));
  }
   ...
   * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
   *
   * @param type String representation of the UART type which mayb be "none", "8250", "16450", "16550", "16550A", "16650",
   * "16550V2" or "16750".
   ...
   * Extension to CommAPI This is an extension to CommAPI. It may not be supported on all operating systems.
   *
   * @return type String representation of the UART type which mayb be "none", "8250", "16450", "16550", "16550A", "16650",
   * "16550V2" or "16750".
   * @throws UnsupportedCommOperationException
   */
  @Override
   ...
   *
   * @param BaudBase The clock frequency divided by 16. Default BaudBase is 115200.
   * @return true on success
