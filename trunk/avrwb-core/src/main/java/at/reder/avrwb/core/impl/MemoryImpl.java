...
final class MemoryImpl implements Memory
{
  ...
  MemoryImpl(String id, String name, ByteOrder byteOrder, int size, int start, int initValue)
  {
    this.id = id;
    this.name = name;
...
//  {
//    return data.getDouble(checkAddress(address));
//  }
//
//  @Override
//  public void setDoubleAt(int address, double d)
//  {
//    data.putDouble(checkAddress(address), d);
//  }

  private int readCharacters(char[] buffer, int offset, int numCharacters, Charset charSet, ByteBuffer dataView)
  {
  ...
  @Override
  public int readCharacters(int address, char[] buffer, int offset, int numCharacters, Charset charSet)
  {
    return readCharacters(buffer, offset, numCharacters, charSet, getBufferAtAddress(address));
  }

  StringBuilder readString(int address, Charset charSet, int bufSize)
  {
    Objects.requireNonNull(charSet, "charSet==null");
  ...
  @Override
  public StringBuilder readString(int address, Charset charSet)
  {
    return readString(address, charSet, 1024);
  }

  @Override
  public int writeCharacters(int address, char[] buffer, int offset, int toWrite, Charset charSet)
  {
    Objects.requireNonNull(buffer, "buffer==null");
    Objects.requireNonNull(charSet, "charSet==null");
    final int ra = checkAddress(address);
